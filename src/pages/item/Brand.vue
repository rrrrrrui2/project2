  ...
  export default {
    ...
    methods: {
      getDataFromServer() { // 从服务的加载数的方法。
        // 发起请求
        this.$http.get("/item-services/brand/page", {
          params: {
      ...
      editBrand(oldBrand){
        // 根据品牌信息查询商品分类
        this.$http.get("/item-services/brand/bid/" + oldBrand.id)
          .then(({data}) => {
            // 修改标记
            this.isEdit = true;
            // 控制弹窗可见：
      ...
      delete(oldBrand){
        // 根据品牌信息查询商品分类
        this.$http.get("/item-services/brand/delete/" + oldBrand.id)
          .then(() => {
