    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-else @click="UpGoods(props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      getDataFromServer() { // 从服务的加载数的方法。
        // 发起请求
        this.$http.get("/item-services/spu/page", {
          params: {
      ...
      async editGoods(oldGoods) {
        // 发起请求，查询商品详情和skus
        oldGoods.spuDetail = await this.$http.loadData("/item-services/spu/detail/" + oldGoods.id);
        oldGoods.skus = await this.$http.loadData("/item-services/sku/list?id=" + oldGoods.id);
        // 修改标记
        this.isEdit = true;
        // 控制弹窗可见：
        this.show = true;
      ...
      OffGoods(id){
        this.$message.confirm("确认要下架该商品吗？")
          .then(() => {
            this.$http.get("/item-services/spu/off/" + id)
              .then(() => {
      ...
      UpGoods(id){
        this.$message.confirm("确认要上架该商品吗？")
          .then(() => {
            this.$http.get("/item-services/spu/up/" + id)
              .then(() => {
      ...
      deleteGoods(id){
        this.$message.confirm("确认要删除该商品吗？")
          .then(() => {
            this.$http.delete("/item-services/spu/delete/" + id)
              .then(() => {
