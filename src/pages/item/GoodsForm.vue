          ...
          <v-form v-model="valid" ref="basic">
            <v-layout row>
              <v-flex xs5>
                <!--商品分类-->
                <v-cascader
                  url="/item-services/category/list"
                  required
                  showAllLevels
    ...
    "goods.categories": {
      ...
      handler(val) {
        ...
        if (val && val.length > 0) {
          // 根据分类查询品牌
          this.$http
            .get("/item-services/brand/cid/" + this.goods.categories[2].id)
            .then(({ data }) => {
              this.brandOptions = data;
            });
          // 根据分类查询规格参数
          this.$http
            .get("/item-services/spec/params?cid=" + this.goods.categories[2].id)
            .then(({ data }) => {
              let specs = [];
              let template = [];
              if (this.isEdit){
