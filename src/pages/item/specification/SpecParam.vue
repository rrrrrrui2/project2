...
export default {
  ...
  methods: {
    loadData() {
      this.$http
        .get("/item-services/spec/params?gid=" + this.group.id)
        .then(({ data }) => {
          data.forEach(p => {
    ...
    deleteParam(id) {
        ...
        .then(() => {
            this.$http.delete("/item-services/spec/param/delete/" + id)
            .then(() => {
    ...
    save(){
        ...
        this.$http({
            ...
            url: this.isEdit ? '/item-services/spec/param/edit' : '/item-services/spec/param/add',
            data: p,
        }).then(() => {
            // 关闭窗口
            this.show = false;
